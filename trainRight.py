...
def train_syndiff(rank, gpu, args):
    ...
    data_loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True, drop_last = True)

    #val_sampler = torch.utils.data.distributed.DistributedSampler(dataset_val, num_replicas=args.world_size, rank=rank)

    data_loader_val = torch.utils.data.DataLoader(dataset=dataset_val, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True, drop_last = True)
    ...
    disc_diffusive_1 = Discriminator_large(nc = 2, ngf = args.ngf, t_emb_dim = args.t_emb_dim, act=nn.LeakyReLU(0.2)).to(device)
    ...
    #ddp
    gen_diffusive_1 = nn.parallel.DistributedDataParallel(gen_diffusive_1, device_ids=[gpu]).to(device)
    #gen_diffusive_2 = nn.DataParallel(gen_diffusive_2, device_ids=[gpu]).to(device)
    #gen_non_diffusive_1to2 = nn.DataParallel(gen_non_diffusive_1to2, device_ids=[gpu]).to(device)
    #gen_non_diffusive_2to1 = nn.DataParallel(gen_non_diffusive_2to1, device_ids=[gpu]).to(device)
    disc_diffusive_1 = nn.parallel.DistributedDataParallel(disc_diffusive_1, device_ids=[gpu]).to(device)
    #disc_diffusive_2 = nn.DataParallel(disc_diffusive_2, device_ids=[gpu]).to(device)

    #disc_non_diffusive_cycle1 = nn.DataParallel(disc_non_diffusive_cycle1, device_ids=[gpu]).to(device)
    #disc_non_diffusive_cycle2 = nn.DataParallel(disc_non_diffusive_cycle2, device_ids=[gpu]).to(device)
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            x1_t, x1_tp1 = q_sample_pairs(coeff, real_data1, t1.to(device)
            ...
            x1_0_predict_diff = gen_diffusive_1(torch.cat((x1_tp1.detach(),x2),axis=1), t1, latent_z1)
            ...
            if iteration % 100 == 0:
                if rank == 0:
                    print('epoch {} iteration {}, G Loss: {}, D loss: {}'.format(epoch, iteration, errG_adv.item(), errD.item()))
        ...
        if rank == 0:
            ...
            x1_t = torch.cat((torch.randn_like(real_data1),x2),axis=1)
            fake_sample1 = sample_from_model(pos_coeff, gen_diffusive_1, args.num_timesteps, x1_t, T, args)
            fake_sample1 = torch.cat((x2, fake_sample1),axis=-1)
        ...
        #    x1_t = torch.randn_like(real_data) #original line: x1_t = torch.cat((torch.randn_like(real_data),source_data),axis=1)
        #    #diffusion steps
        #    fake_sample1 = sample_from_model(pos_coeff, gen_diffusive_1, args.num_timesteps, x1_t, T, args)
...
if __name__ == '__main__':
    ...
    if size > 1:
        ...
        #train_syndiff(args.local_rank + args.node_rank * args.num_process_per_node, args.local_rank, args)
